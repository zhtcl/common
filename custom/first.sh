...
if [[ "${GIT_REPOSITORY}" =~ (zhtcl/build-actions|zhtcl/autobuild) ]]; then
  rm -rf backups
...
curl -fsSL https://raw.githubusercontent.com/zhtcl/common/main/common.sh -o common.sh
if [[ $? -ne 0 ]]; then
  wget -q https://raw.githubusercontent.com/zhtcl/common/main/common.sh -O common.sh
fi
...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  export GIT_REPOSITORY="zhtcl/build-actions"
  sudo rm -rf build common.sh
