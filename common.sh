...
cd ${GITHUB_WORKSPACE}
bash <(curl -fsSL https://raw.githubusercontent.com/zhtcl/common/main/custom/first.sh)
}
...
function Diy_update() {
bash <(curl -fsSL https://raw.githubusercontent.com/zhtcl/common/main/custom/ubuntu.sh)
if [[ $? -ne 0 ]];then
...
dockerd,miniupnpd,luci-app-torbp,*luci-app-passwall*,luci-app-passwall,luci-app-passwall2"
...
dockerd,luci-app-bmtedge,luci-app-bypass,luci-app-smartdns,smartdns,adguardhome,alist,chinadns-ng,cpulimit,ddns-go,filebrowser,gost,iptvhelper,lua-maxminddb,luci-lib-iform,lucky,mosdns,msd_lite,natter,oaf,open-app-filter,oscam,udp2raw,upx-static,v2dat,v2ray-geodata"
t=(${z//,/ })
for x in ${t[@]}; do \
...
if [[ `grep -c "CONFIG_PACKAGE_luci-theme-argon=y" ${HOME_PATH}/.config` -eq '1' ]]; then
  ...
  curl -fsSL https://raw.githubusercontent.com/zhtcl/openwrt-package/usb/argon/jpg/7.jpg -o ${HOME_PATH}/files/www/luci-static/argon/background/argon.jpg
  if [[ $? -ne 0 ]]; then
    echo "拉取文件错误,请检测网络"
    exit 1
  fi
...
if [[ -z "${amlogic_kernel}" ]]; then
  curl -fsSL https://github.com/zhtcl/common/releases/download/API/${kernel_usage}.api -o ${HOME_PATH}/${kernel_usage}.api
